/* Food Truth Scanner — personalization feature. */
...
function forYouSummary(product, targets, profile) {
  ...
  if (pct.protein_g != null && pct.protein_g >= 20) return `Good protein contribution at ${pct.protein_g}% of your daily target.`;
...
function dailyTargetBar(label, value, target, unit) {
  ...
  return `<div class="macro-bar-row"><div class="macro-bar-label"><span>${label}</span><span class="amt">${value}${unit} · ${pct}% of daily target</span></div><div class="macro-bar-track"><div class="macro-bar-fill" style="width:${width}%;background:${macroBarColor(pct)};"></div></div></div>`;
}
...
function renderForYouPanel(product) {
  ...
  const sugarNote = state.profile.targetMode !== "custom"
    ...
    : "";
  return `<div class="panel for-you-panel"><div class="panel-title">For You</div><div class="panel-sub">${forYouSummary(product, targets, state.profile)}<br>${mode}</div>${dailyTargetBar("Calories", product.nutrition.calories, targets.calories, " kcal")}${dailyTargetBar("Sugar", product.nutrition.sugar_g, targets.sugar_g, "g")}${dailyTargetBar("Protein", product.nutrition.protein_g, targets.protein_g, "g")}${dailyTargetBar("Sodium", product.nutrition.sodium_mg, targets.sodium_mg, "mg")}${sugarNote}</div>`;
}
...
function renderTargetSettings() {
  ...
  return `<div class="panel target-settings-panel"><div class="panel-title">Daily nutrition targets</div><div class="panel-sub">Used only for the personalized “For You” context. The Food Truth Score itself does not change.</div><div class="target-mode-row"><label><input type="radio" name="target-mode" ${!custom ? "checked" : ""} onchange="setTargetMode('recommended')"> Recommended</label><label><input type="radio" name="target-mode" ${custom ? "checked" : ""} onchange="setTargetMode('custom')"> Set my own</label></div>${custom ? `<div class="target-custom-grid">${targetInput("Calories", "calories", active.calories, "kcal")}${targetInput("Protein", "protein_g", active.protein_g, "g")}${targetInput("Total sugar limit", "sugar_g", active.sugar_g, "g", "optional")}${targetInput("Sodium limit", "sodium_mg", active.sodium_mg, "mg")}</div><div class="panel-sub" style="margin-top:8px;">Only set a sugar limit here if you intentionally want to track <b>total</b> sugar from product labels.</div>` : `<div class="panel-sub" style="margin:10px 0 0;">Recommended: ${recommended.calories} kcal · ${recommended.protein_g}g protein · ${recommended.sodium_mg}mg sodium. No default total-sugar target is assumed.</div>`}</div>`;
}
...
function setTargetMode(mode) {
  ...
  if (state.profile.targetMode === "custom" && !state.profile.customTargets) {
    ...
    state.profile.customTargets = { calories: r.calories, protein_g: r.protein_g, sugar_g: null, sodium_mg: r.sodium_mg };
  }
  render();
}
...
function installPersonalization() {
  const baseCuratedResult = renderResultOverlay;
  renderResultOverlay = function(id) {
    const product = getProduct(id);
    let html = baseCuratedResult(id);
  ...
  if (typeof window.renderOffResultOverlay === "function") {
    const baseOffResult = window.renderOffResultOverlay;
    window.renderOffResultOverlay = function(product) {
      let html = baseOffResult(product);
