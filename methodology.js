...
function renderFoodTruthMethodologySheet() {
  return `<div class="sheet-overlay" onclick="if(event.target===this) closeSheet()">
    <div class="sheet">
      ...
      <div class="weight-row"><span>Solid-food energy / fat density</span><b>up to −15 pts</b></div>
      ...
      <p>For solid foods, the model also applies a capped energy-density / total-fat safeguard. It starts only above moderate density and is designed to stop fried, calorie-dense snacks from looking healthy simply because they are low in sugar. Normal foods such as plain yoghurt are below these thresholds and receive no density penalty.</p>
      ...
      <p><b>Processing is scored through one shared classifier.</b> It looks for standardized signals such as deep-fried/fried snacks, ultra-processed / reconstituted formulation, non-nutritive sweeteners and sugar alcohols, preservatives, emulsifiers / stabilizers, artificial colours / flavours, and unusually high additive load. Related additives are grouped so the same type of signal is not counted repeatedly.</p>
      <p>Curated products provide manually researched ingredient evidence. Open Food Facts products provide ingredients plus NOVA and additive metadata when available. External scores can be provisional when evidence is incomplete; the data-confidence rating communicates how much weight to place on that score rather than treating missing fields as zero.</p>
